...
def check_whisper():
    ...
    if not ask("下载 Whisper 语音识别模型", "y"):
        print("已跳过语音识别安装：语音识别功能将【不可用】（文字对话不受影响）。")
        return False
    try:
        code = (
        ...
        if os.path.exists(model_bin):
            ...
            return True
        print("Whisper 模型下载后文件不完整，语音识别可能不可用。")
        return False
    except Exception as e:
        print(f"Whisper 模型下载失败：{e}")
        print("语音识别将【不可用】（可在之后手动下载模型到 models/whisper）。")
        return False
...
def check_tts():
    ...
    if os.path.exists(LAUNCHER_CFG):
        try:
            ...
            print(f"读取 launcher_config.json 失败：{e}")
...
def install_ollama():
    ...
    try:
        ...
        urllib.request.urlretrieve("https://gh-proxy.org/https://github.com/ollama/ollama/releases/download/v0.32.14/OllamaSetup.exe", exe) #这里使用gh加速方便国内用户下载，版本0.32.14
...
def pull_ollama_model(ollama):
    ...
    if ask("是否拉取默认模型 qwen3.5:9b（约 5GB，需联网）？", "y"):
        try:
            ...
            print(f"模型拉取失败：{e}（可稍后手动执行 ollama pull qwen3.5:9b）")
            return False
    print("已跳过模型拉取：请在启动前执行 ollama pull qwen3.5:9b。")
    return False
...
def main():
    print("=" * 52)
    print("  小笼洛包 1.0 · 部署程序")
    print("  bilibili@我叫清少（UID:478929333） 重写")
    print("=" * 52)
    ...
    if not python:
        print("未检测到 Python。")
        print("请您先前往 https://www.python.org/downloads/windows 下载 Python 3.9-3.14 并在安装时勾选“Add python.exe to PATH”")
        sys.exit(1)
